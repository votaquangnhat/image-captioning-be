from modules import FeatureExtractor,Encoder,Decoder,Config
...
def infer(image):
    ...
    decoder.load_state_dict(torch.load("decoder.pth", map_location=torch.device('cpu'), weights_only=True))
    encoder.load_state_dict(torch.load("encoder.pth", map_location=torch.device('cpu'), weights_only=True))
